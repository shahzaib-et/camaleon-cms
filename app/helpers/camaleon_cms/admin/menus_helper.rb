  ...
  def admin_menus_add_commons
    ...
    current_site.post_types.eager_load(:metas).visible_menu.all.each do |pt|
      ...
      if pt.manage_categories?
        ...
        if pt.name == "Event"
          items_i << {icon: "folder-open", title: "Thema 1", url: post_type_thema1_index } if can? :categories, pt
          items_i << {icon: "folder-open", title: "Thema 2", url: post_type_thema2_index } if can? :categories, pt
          items_i << {icon: "folder-open", title: "Erlebnisart", url: post_type_type_index } if can? :categories, pt
        end
      end
      if pt.manage_tags?
